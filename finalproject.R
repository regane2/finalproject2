...
swift_condensed <- swift %>% 
  filter(album_name %in% albums_to_keep) %>% 
  distinct()
View(swift_condensed)
colnames(swift_condensed)
swift_condensed <- swift_condensed %>% 
  select(artist_name, album_type, album_release_date, danceability, energy, key, loudness, mode, speechiness, acousticness, instrumentalness, liveness, valence, time_signature, duration_ms, explicit, track_name, track_number, album_name, key_mode) %>% 
  distinct() 
